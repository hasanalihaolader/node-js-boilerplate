...
const UserController = {
  getUsers: async (req: Request, res: Response, next: NextFunction) => {
    ...
    return res.status(statusCode).json(
      helper.formatApiResponse(statusCode, message, context)
    );
  },
  ...
  userProfile: async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!token) {
      return res
        .status(401)
        .json(helper.formatApiResponse(401, 'Access Denied. No token provided.'));
    }
    ...
    try {
      ...
      return res.status(200).json(
        helper.formatApiResponse(200, 'User profile fetched successfully', { user })
      );
    } catch (error) {
      return res
        .status(400)
