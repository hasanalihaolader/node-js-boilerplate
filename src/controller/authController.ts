...
const AuthController = {
  ...
  login: async (req: Request, res: Response, next: NextFunction): Promise<Response> => {
    try {
      ...
      return res
        .status(200)
        .json(helper.formatApiResponse(200, 'Token generation successful', { token }));
    } catch (error) {
      return res
        .status(500)
  ...
  refresh: async (req: Request, res: Response, next: NextFunction): Promise<Response> => {
    ...
    if (!token) {
      return res
        .status(401)
        .json(helper.formatApiResponse(401, 'Access Denied. No refresh token provided.'));
    }
    ...
    try {
      ...
      return res
        .status(200)
        .json(helper.formatApiResponse(200, 'Token generation successful', { token: accessToken }));
    } catch (error) {
      return res
        .status(400)
